...
import { Send, Copy, Mail, Settings, Sparkles, User, Building, MessageSquare, Clock, Heart, Briefcase, Shield, Smile, LogOut, UserPlus, LogIn, Users, AlertCircle } from 'lucide-react';
...
const EmailGeneration = ({ user }) => {
    ...
    const checkCanGenerate = () => {
        ...
        const hasPackage = subscriptionData.package_id && subscriptionData.package_generations_remaining > 0;
        ...
        if (isPro && !hasPackage) {
            return {
                allowed: false,
                reason: 'no_package',
                message: 'You\'re on the Pro plan, but you don\'t have an active package yet.',
                userPlan: 'Pro',
                hasPackage: false
            };
        }
                                    ...
                                    <div>
                                        ...
                                        {subscriptionData.plan_name === 'Free' ? (
                                            <p className="text-green-100 text-sm">
                                                {subscriptionData.generation_limit - (usageData?.generations_count || 0)}/{subscriptionData.generation_limit} generations remaining this month
                                            </p>
                                        ) : (
                                            <p className="text-amber-100 text-sm">
                                                {subscriptionData.package_id && subscriptionData.package_generations_remaining > 0 ? (
                                                    <>
                                                        {subscriptionData.package_generations_remaining} generations left in {subscriptionData.package_name} package
                                                    </>
                                                ) : (
                                                    <>
                                                        No active package. <button onClick={() => router.push('/packages')} className="underline font-semibold hover:text-white">Purchase a package</button> to start generating.
                                                    </>
                                                )}
                                            </p>
                                        )}
